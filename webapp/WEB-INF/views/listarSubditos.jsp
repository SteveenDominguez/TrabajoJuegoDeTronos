	...
	<table border="1" id="myTable">
		...
		<tbody>
			<c:forEach items="${subditos }" var="subdito">
				<tr>
					<td>${subdito.id}</td>
					<td>${subdito.nombre}</td>
					<td>${subdito.numHijos}</td>
					<td>${subdito.problema}</td>
					<td>${subdito.padre}</td>
					<td>${subdito.region}</td>
					<td><a href="editarSubdito/${subdito.id}"
						class="btn btn-success btn-sm" role="button" title="Edit"> <span
							class="glyphicon glyphicon-pencil"></span>editar
					</a> <a href="eliminarSubdito/${subdito.id}"
						onclick='return confirm("¿Estas seguro eliminar este subdito?")'
