...
<body>
	...
	<table border="1" id="myTable">
		...
		<tbody>
			<c:forEach items="${guerreros }" var="guerrero">
				<tr>
					<td>${guerrero.id}</td>
					<td>${guerrero.nombre}</td>
					<td>${guerrero.especialidad}</td>
					<td>${guerrero.cargo}</td>
					<td>${guerrero.numAsesinatos}</td>
					<td>${guerrero.interes}</td>
					<td>${guerrero.region}</td>
					<td><a href="editarGuerrero/${guerrero.id}"
						class="btn btn-success btn-sm" role="button" title="Edit"> <span
							class="glyphicon glyphicon-pencil"></span>editar
					</a> <a href="eliminarGuerrero/${guerrero.id}"
						onclick='return confirm("¿Estas seguro eliminar este guerrero?")'
